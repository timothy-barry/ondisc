...
2. Create an `ondisc_matrix` representing the gene expression and gRNA indicator data.
3. Select which genes and gRNAs to use in the analysis.
4. Compute the cell-wise covariate matrix.
5. Perform quality control on the cells.

Before proceeding, we load all required packages.
...
Now that we have downloaded the data, we can put the gene expression and gRNA indicator data into `ondisc_matrix` objects. First, we call `create_ondisc_matrix_from_mtx` on the gene expression data. This function takes about 10-20 minutes to run on a Macbook Pro laptop.
...
Now, we can put the perturbation data into an `ondisc_matrix` by calling `create_ondisc_matrix_from_mtx` on `perturbations.mtx` and associated metadata files. The function takes a few seconds to run.
...
Next, we select the genes, gRNAs, and gene-gRNA pairs to use in the analysis. We exclude genes that are expressed in fewer than 1,000 cells and gRNAs that are expressed in fewer than 100 cells.
...
We end up keeping 10,870 of 32,738 genes and 1,560 of 1,561 gRNAs. Next, we select the gene-gRNA pairs to analyze.

```{r}
original_pairs <- read_tsv(file = paste0(raw_data_dir, "/GSE120861_gene_gRNAgroup_pair_table.pilot.txt"),
                           col_types = c("___c____c____"), col_names = TRUE) %>% 
  rename("gRNA_id" = "gRNAgroup", "gene_id" = "ENSG.targetgene")
head(original_pairs)
```

```{r}
my_genes <- get_feature_ids(expressions)
my_gRNAs <- get_feature_ids(perturbations)

my_pairs <- filter(.data = original_pairs, gRNA_id %in% my_gRNAs & gene_id %in% my_genes)
saveRDS(my_pairs, paste0(processed_data_dir, "/pairs.rds"))
```

# 4. Cell-wise covariate matrix

```{r}
crispr_experiment <- multimodal_ondisc_matrix(list(expressions = expressions_sub, perturbations = perturbations_sub))
```
