test_that("check covariates", {
  for (i in seq(1, n_datasets)) {
    ...
    # look at cell-wise stats for in memory init
    in_memory_cell_covariates <- cov_odms_from_memory[[i]]$cell_covariates
    for (curr_col in colnames(in_memory_cell_covariates)) {
      # n nonzero
      if (curr_col == "n_nonzero") {
        test <- Matrix::colSums(r_mat >= 1)
      } else if (curr_col == "n_umis") {
      ...
      expect_equal(test, in_memory_cell_covariates[,curr_col])
    }
    ...
    # next, look at gene-wise stats for in memory init
    in_memory_feature_covariates <- cov_odms_from_memory[[i]]$feature_covariates
    for (curr_col in colnames(in_memory_feature_covariates)) {
      if (curr_col == "n_nonzero") {
        test <- Matrix::rowSums(r_mat >= 1)
      } else if (curr_col == "mean_expression") {
        test <- Matrix::rowMeans(r_mat)
      ...
      expect_equal(test, in_memory_feature_covariates[,curr_col], tolerance = 0.01)
    }
  }
})
